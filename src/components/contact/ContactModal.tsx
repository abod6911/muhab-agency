...
• Brief: ${formData.brief || 'Let\'s discuss on call'}`;
  ...
  return (
    <AnimatePresence>
      <div 
        className="fixed inset-0 z-[200] flex items-center justify-center p-2 sm:p-4 overflow-y-auto overscroll-contain select-none"
      ...
      >
        ...
        <motion.div
          ...
          className="relative w-full max-w-xl bg-gradient-to-b from-[#072418]/98 via-[#03150d]/98 to-[#010a05] border border-emerald-500/35 rounded-[1.75rem] sm:rounded-[2rem] shadow-[0_25px_80px_rgba(0,0,0,0.9),0_0_40px_rgba(166,255,46,0.1)] z-10 p-4 sm:p-6 sm:px-7 my-auto max-h-[94vh] overflow-y-auto [&::-webkit-scrollbar]:hidden"
        >
          {/* Top Luminous Beam */}
          ...
          <div className="absolute -top-24 -left-24 w-60 h-60 bg-[#a6ff2e]/10 rounded-full blur-3xl pointer-events-none" />
          ...
          <button
            ...
            className="absolute top-4 sm:top-5 rtl:left-4 rtl:sm:left-5 rtl:right-auto ltr:right-4 ltr:sm:right-5 ltr:left-auto w-8 h-8 rounded-full bg-white/5 hover:bg-[#a6ff2e]/20 text-slate-400 hover:text-[#a6ff2e] border border-white/10 hover:border-[#a6ff2e]/40 flex items-center justify-center transition-all cursor-pointer z-20 group"
                ...
                <div className="grid grid-cols-3 gap-2">
                  {budgetOptions.map((budget, i) => (
                    <button
                      ...
                      className={`px-2 py-2 rounded-xl text-[11px] sm:text-xs font-bold border transition-all cursor-pointer relative overflow-hidden text-center flex flex-col items-center justify-center gap-0.5 ${
                        formData.budget === budget
                          ? 'bg-gradient-to-r from-[#a6ff2e] to-[#bbfd5c] text-[#020a06] border-[#a6ff2e] shadow-[0_0_18px_rgba(166,255,46,0.35)] scale-[1.02]'
                          : 'bg-[#0a2318]/50 text-slate-300 border-emerald-500/20 hover:border-emerald-500/40 hover:bg-[#0a2318]/80'
                      }`}
                    >
                      <span>{budget}</span>
                      {i === 1 && (
                        <span className={`text-[9px] font-bold ${
                          formData.budget === budget ? 'text-[#020a06]/85' : 'text-[#a6ff2e]'
                        }`}>
                ...
                <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                  ...
                  {quickTags.map((tag, idx) => (
                    <button
                      ...
                      onClick={() => {
                        audioSynth.playTelemetryTick();
                        const separator = formData.brief ? ' • ' : '';
                        setFormData({ ...formData, brief: formData.brief + separator + tag });
                      }}
                      className="px-2 py-0.5 rounded-lg text-[10px] font-medium bg-emerald-950/40 hover:bg-[#a6ff2e]/20 text-slate-300 hover:text-[#a6ff2e] border border-emerald-500/20 hover:border-[#a6ff2e]/40 transition-colors cursor-pointer"
                    >
                      + {tag}
                    </button>
                  ))}
